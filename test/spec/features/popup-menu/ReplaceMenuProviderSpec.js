...
describe('features/popup-menu - replace menu provider', function() {
  ...
  describe('data object - collection marker', function() {
    ...
    it('should activate marker of linked data object reference', inject(function(elementRegistry) {
      ...
      // given
      var dataObjectReference = elementRegistry.get('DataObjectReference_1');
      var linkedDataObjectReference = elementRegistry.get('DataObjectReference_2');
      ...
      openPopup(linkedDataObjectReference);
    ...
    // Incomplete
    it('should update all references to data object', inject(function(elementRegistry, modeling, eventBus) {

      // given
      var dataObjectReference = elementRegistry.get('DataObjectReference_1');
      var linkedDataObjectReference = elementRegistry.get('DataObjectReference_2');
      var dataObject = dataObjectReference.businessObject.dataObjectRef;

      var changedListener = sinon.spy(function(event) {
        console.log(event);

        // expect(event.elements).to.eql('');
      });

      // assume
      expect(dataObject).to.eql(linkedDataObjectReference.businessObject.dataObjectRef);

      // when
      eventBus.on('commandStack.changed', changedListener);
